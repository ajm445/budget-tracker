-- Transactions table for japan-working-holiday-app
...
CREATE TABLE public.transactions (
  ...
  date TEXT NOT NULL,
  currency TEXT NOT NULL CHECK (currency = ANY (ARRAY['KRW'::text, 'USD'::text, 'JPY'::text])),
...
COMMENT ON COLUMN public.transactions.currency IS '거래 통화 (KRW, USD, JPY)';
COMMENT ON COLUMN public.transactions.amount_in_krw IS '원화 기준 환산 금액';
